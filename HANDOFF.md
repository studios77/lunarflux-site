...
5. **상담 채널이 폼과 전화 둘뿐입니다** — 2026-08-08 에 채널톡을 걷어냈습니다.
   웹 상담 창구가 없는 상태이므로 대체 도구를 정해야 합니다. 조사 결과 국내
   B2B 에서 결정적인 변수는 **카카오 상담톡 연동 가능 여부**였습니다. 카카오·
   네이버 톡톡을 네이티브로 받는 건 사실상 채널톡뿐이고, Crisp·Intercom·
   Chatwoot 은 카카오를 지원하지 않습니다(Chatwoot 은 브릿지를 직접 만들어야 함).
   도구를 새로 고르기 전에 **카카오톡 채널을 먼저 개설**하는 편이 낫습니다 —
   개설이 무료이고, 어떤 도구를 고르든 문의가 고객 폰 알림으로 도달합니다.
