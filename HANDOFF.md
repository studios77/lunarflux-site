...
- P0 4건: 메가메뉴 키보드/터치, JS 실패 폴백, 상담 위젯 전역화, Footer 누락 (`47cf1cb`)
...
5. **SalesIQ 대시보드 설정 2건** — 코드는 끝났고 아래는 Zoho 관리자 일입니다.
   - **모바일 앱 알림 켜기.** 응답 속도의 대부분이 여기서 갈립니다.
   - **월 100 대화 사용량 알림 걸기.** 무료 플랜 상한이라 소진되면 그 달
     채팅이 막힙니다. 미리 알림을 받아 두는 편이 안전합니다.

### 마친 것 (재확인 불필요)

- **라이브 채팅 — Zoho SalesIQ** (2026-08-09) — 상담 도구는 SalesIQ 하나이고,
  **무료 플랜**(상담원 3명 · 월 대화 100건)입니다. 메일을 이미 Zoho 로 쓰고
  있어 벤더와 계정이 늘지 않았습니다. 이전에 붙였던 위젯들의 소스는 전부
  제거했습니다.

  ```
  components/SalesIq.tsx            위젯 로더 (전역, layout 에서 마운트)
  components/SalesIqOpenButton.tsx  contact 페이지에서 채팅창 열기
  lib/site.ts  SALESIQ              위젯 코드 · 스크립트 URL
  ```

  **위젯 코드는 전역이 아니라 스크립트 URL 의 `?wc=` 로 넘어갑니다.** 예전
  스니펫은 `$zoho.salesiq.widgetcode` 에 넣는 방식이었고 문서와 블로그에는
  그 형태가 많이 남아 있습니다. 대시보드가 주는 실제 스니펫을 기준으로
  맞추세요. 전역(`$zoho.salesiq`)을 스크립트보다 먼저 세우는 순서는 그대로
  지켜야 합니다 — 스크립트가 로드되며 그 전역에 API 를 붙입니다.

  **로딩을 유휴 시간까지 미룹니다.** 상담 위젯은 방문자가 도착하자마자 필요한
  것이 아닌데, 마운트 즉시 외부 번들을 받으면 첫 화면 렌더와 경쟁합니다.
  `requestIdleCallback` 뒤로 보내고 미지원 브라우저는 타이머로 대체하며,
  3초 timeout 으로 계속 바쁜 페이지에서도 결국 뜨도록 보장합니다. 그래서
  화면이 그려지고 1~3초쯤 뒤에 버튼이 나타납니다 — 고장이 아닙니다.

  contact 버튼은 "우측 하단을 클릭하세요" 로 위치를 설명하지 않고 그 자리에서
  엽니다. 로딩을 미루므로 방문자가 먼저 누르면 전역이 아직 없을 수 있어
  최대 6초 기다렸다가 열고, 끝내 못 뜨면(광고 차단 등) 이메일을 안내합니다 —
  눌러도 아무 일이 없는 버튼은 두지 않습니다.

  `SALESIQ.widgetCode` 가 비면 위젯과 contact 카드가 통째로 빠집니다. 빈 값·
  채운 값 양쪽으로 빌드해 확인했고, 위젯 스크립트 URL 이 `HTTP 200`
  (`text/javascript`) 으로 응답하는 것도 확인했습니다.

  > **`Window` 확장 타입에 `requestIdleCallback` 을 다시 선언하지 마세요.**
  > DOM 타입에 이미 필수로 있어 교집합이 필수가 되고, `if (ric)` 가 "항상 참"
  > 으로 좁혀져 TS2774 로 빌드가 깨집니다. 존재 확인은 `typeof` 로 합니다.

  > **상담 위젯을 바꿀 때는 `/contact` 문구를 함께 고치세요.** 예전에 위젯을
  > 지우면서 "화면 우측 하단의 채팅 버튼을 클릭하시면" 카드를 남겨 두어,
  > 없는 버튼을 가리킨 적이 있습니다. 그래서 지금은 카드가 위젯 코드 유무에
  > 따라 통째로 붙었다 빠지도록 해 두었습니다.

  > **Zoho 조직 이름(`lunarfluxai`)은 코드와 무관합니다.** 위젯은 `?wc=` 값
  > 하나로 식별됩니다. 대신 **Brand 에 등록된 웹사이트 주소**가 `lunarflux.ai`
  > 인지 확인하세요 — 여기가 틀리면 스크립트는 받아지는데 채팅창이 뜨지
  > 않습니다.

- **상담 도구 선정에서 헛돈 것 — 반복하지 마세요** (2026-08-09)

  하루 동안 채널톡 → 카카오톡 채널 → (전부 제거) → Freshchat → SalesIQ 로
  다섯 번 갈아 끼웠습니다. 원인은 대부분 **"무료" 표기를 검증하지 않은 것**입니다.

  | 도구 | 실제 |
  |---|---|
  | Drift | **서비스 종료(sunset) 중**, 신규 가입 불가. 최소 연 $30,000 |
  | Freshchat | "무료 플랜(Sprout)" 으로 소개되지만 실제로는 **15일 체험판** |
  | 채널톡 | 무료 플랜 있음. 국내에서 **카카오 상담톡·네이버 톡톡을 한 인박스로 받는 사실상 유일한 경로** |
  | tawk.to | 상담원·대화 **무제한** 영구 무료. 위젯에 브랜딩 노출(제거 유료) |
  | Zoho SalesIQ | 상담원 3명 · 월 100 대화 영구 무료 ← **채택** |

  **리스티클의 "free plan" 표기는 믿을 게 못 됩니다.** 제휴 수수료가 걸린 글이
  대부분이라 체험판을 무료 플랜으로 적습니다. 도구를 새로 고를 때는 공식 가격
  페이지에서 **기한과 상한**을 직접 확인하세요.

  카카오톡 채널(`_vbYBX`)은 만들어 두었고 채팅도 켜 두었지만 사이트에서 링크를
  뺐습니다. 접을지 남길지는 별도 판단입니다. 카카오 채널을 새로 만들면
  **"채팅이 불가능한 프로필입니다"** 를 만날 수 있는데, 관리자센터 → 채팅 →
  채팅 설정에서 채팅을 켜면 해결됩니다.
...
- **SEO 설정 점검** (2026-08-08) — 라이브에서 직접 확인했습니다. title·description·
  ...
  슬래시 일치까지 정상입니다. 여기서 나온 지적 사항(og:image 누락, 홈 h1,
  sitemap lastmod)은 같은 날 전부 처리했습니다.
