...
5. **Freshchat 토큰 한 줄 채우기** — 위젯 코드는 들어가 있는데 계정이 아직
   없어 **화면에는 나오지 않습니다.**

   ```
   Freshchat 관리자 → Settings → Channels → Web Chat  (토큰·호스트가 함께 표시)
   lib/site.ts  FRESHCHAT = { token: '...', host: 'https://wchat.freshchat.com' }
   ```

   호스트는 리전마다 다릅니다(미국·EU·인도·호주). **관리자 화면에 적힌 값을
   그대로 쓰세요** — 틀리면 위젯이 조용히 뜨지 않습니다. 토큰이 비어 있으면
   위젯과 contact 카드가 통째로 빠집니다. 토큰은 브라우저에 노출되는 값이라
   비밀이 아니며, 그래서 Cloudflare Secret 이 아니라 저장소에 둡니다.

   무료 플랜으로 가입해 두었습니다(2026-08-08).

### 마친 것 (재확인 불필요)

- **Freshchat 웹 위젯** (2026-08-08) — 상담 도구는 Freshchat 하나입니다.
  이전에 쓰던 위젯들의 소스는 전부 제거했습니다.

  ```
  components/FreshChat.tsx            위젯 로더 (전역, layout 에서 마운트)
  components/FreshChatOpenButton.tsx  contact 페이지에서 채팅창 열기
  lib/site.ts  FRESHCHAT             토큰·호스트
  ```
  ...
  **로딩을 유휴 시간까지 미룹니다.** 상담 위젯은 방문자가 도착하자마자 필요한
  ...
  3초 timeout 으로 계속 바쁜 페이지에서도 결국 뜨도록 보장합니다.

  contact 버튼은 "우측 하단을 클릭하세요" 로 위치를 설명하지 않고 그 자리에서
  엽니다. 로딩을 미루므로 방문자가 먼저 누르면 `fcWidget` 이 아직 없을 수
  있어 최대 6초 기다렸다가 열고, 끝내 못 뜨면(광고 차단 등) 이메일을
  안내합니다 — 눌러도 아무 일이 없는 버튼은 두지 않습니다.

  토큰이 비면 위젯과 contact 카드가 통째로 빠집니다. 빈 값·채운 값 양쪽으로
  빌드해 확인했습니다.

  > **`FcWindow` 에 `requestIdleCallback` 을 다시 선언하지 마세요.** DOM 타입에
  > 이미 필수로 있어 교집합이 필수가 되고, `if (ric)` 가 "항상 참" 으로 좁혀져
  > TS2774 로 빌드가 깨집니다. 존재 확인은 `typeof` 로 합니다.

  > **상담 위젯을 바꿀 때는 `/contact` 문구를 함께 고치세요.** 예전에 위젯을
  > 지우면서 "화면 우측 하단의 채팅 버튼을 클릭하시면" 카드를 남겨 두어,
  > 없는 버튼을 가리킨 적이 있습니다. 그래서 지금은 카드가 토큰 유무에 따라
  > 통째로 붙었다 빠지도록 해 두었습니다.
