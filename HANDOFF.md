...
5. **카카오톡 채널 마무리** — 사이트 코드 쪽은 끝났고 아래는 카카오 관리자 일입니다.
   - **채널 이름이 공개 페이지에 아직 `lunarfluxal` 로 나옵니다.** 2026-08-08 에
     고쳤다고 하셨는데 `og:title` 은 그대로였습니다. 카카오가 오래 캐싱하는
     것인지, `채널 이름` 이 아니라 `검색용 아이디` 를 고친 것인지 확인이
     필요합니다. 카카오톡에서 채널을 검색해 표시 이름을 직접 보는 게 확실합니다.
     URL(`_vbYBX`)은 이름과 별개라 바꿔도 사이트 코드는 그대로 둡니다.
   - **모바일 앱 알림 켜기.** 응답 속도의 대부분이 여기서 갈립니다.
   - 참고: 채널 페이지는 3.8KB SPA 껍데기라 **바깥에서 상태를 확인할 수
     없습니다.** 채팅 가능 여부와 표시 이름은 직접 눌러 보는 수밖에 없습니다.

### 마친 것 (재확인 불필요)

- **카카오톡 채널 상담 버튼** (2026-08-08) — 채널톡을 걷어낸 자리에 넣었습니다.

  > 붙인 직후 버튼을 누르면 **"채팅이 불가능한 프로필입니다"** 가 떴습니다.
