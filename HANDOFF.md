...
5. **상담 위젯 없음 — 유지할지 결정** — 2026-08-08 에 채널톡과 카카오톡 채널
   버튼을 차례로 걷어냈고, 마지막 판단은 **띄우지 않기** 였습니다. 상담 창구는
   `/contact` 의 문의 폼과 전화 둘입니다. 되돌릴 일이 있으면 아래 커밋을
   참고하세요 — 코드는 git 이력에 그대로 있습니다.

   ```
   채널톡 원본   31a5ade:components/ChatBot.tsx
   카카오 버튼   e16fd6d:components/KakaoChat.tsx
   ```

   카카오 채널(`_vbYBX`)은 살아 있고 채팅도 켜 두었지만 사이트에서 링크를
   빼서 유입 경로가 없습니다. 채널을 접을지 남길지는 별도 판단입니다.

### 마친 것 (재확인 불필요)

- **상담 위젯 전부 제거** (2026-08-08) — 채널톡 → 카카오톡 채널 버튼 →
  둘 다 제거로 마무리했습니다. 지금 사이트에는 떠 있는 상담 위젯이 없습니다.
  외부 스크립트가 사라져 첫 화면 로딩과 경쟁하던 비용도 함께 없어졌습니다.

  > 카카오 버튼을 붙였을 때 **"채팅이 불가능한 프로필입니다"** 가 떴습니다.
  > 링크나 코드 문제가 아니라 채널의 채팅 기능이 꺼져 있어서였고,
  > 관리자센터 → 채팅 → 채팅 설정에서 켜서 해결했습니다. 새 채널을 만들면
  > 다시 만날 수 있으니 기억해 두세요. PC 카카오톡에서만 뜨고 모바일은
  > 되는 경우도 보고돼 있어, 증상을 볼 때 두 환경을 모두 확인하세요.

  ```
  lib/site.ts  KAKAO_CHANNEL_URL = 'https://pf.kakao.com/_vbYBX/chat'
  ```

  `components/KakaoChat.tsx` 는 링크 하나라 **서버 컴포넌트**입니다 —
  `'use client'` 도 외부 스크립트도 없습니다. 채널톡이 전 페이지에서 마운트
  즉시 외부 번들을 받아 첫 화면과 경쟁하던 비용이 0 이 됐습니다.
  버튼은 우측 하단, `ScrollTop` 은 좌측 하단이라 겹치지 않습니다.

  **`KAKAO_CHANNEL_URL` 이 비면 플로팅 버튼과 contact 카드가 통째로 사라집니다.**
  없는 채널로 보내는 링크를 내보내지 않으려는 의도입니다. 빈 값·채운 값 양쪽으로
  빌드해 21개 페이지 전부에서 확인했고, 채널 URL 이 HTTP 200 인 것도 확인했습니다.

- **채널톡 제거** (2026-08-08) — 사용자 결정으로 위젯을 걷어냈습니다.

  ```
  삭제   components/ChatBot.tsx
  수정   app/layout.tsx        <ChatBot /> 와 import 제거
  수정   app/contact/page.tsx  "실시간 채팅 상담(추천)" 카드 삭제
  수정   DEVELOPMENT.md        폴더 구조 트리에서 제거
  ```

  **contact 페이지를 함께 고쳐야 했던 이유** — 그 카드가 "화면 우측 하단의 채팅
  버튼을 클릭하시면" 이라고 안내하고 있었습니다. 위젯만 지우면 없는 버튼을
  가리키게 됩니다. 카드에 있던 운영 시간(평일 10:00-19:00, 긴급 장애 24시간)은
  전화 카드로 옮겼고, "채팅/이메일" 문구는 "위 문의 폼" 으로 바꿨습니다.
  빌드 산출물에 `channel.io`·`ChannelIO` 가 남지 않은 것을 확인했습니다.
  다시 붙일 때는 `app/layout.tsx` 와 `app/contact/page.tsx` 의 주석을 보세요.
