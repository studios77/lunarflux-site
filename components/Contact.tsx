...
export default function Contact() {
  ...
  const contactItems: Array<{
    ...
    { icon: '💬', title: 'Kakao / Telegram', val: '@lunarflux_support' },
    {
      icon: '🕐',
      title: '응답 시간',
      lines: ['전문 엔지니어 검토 후 24시간 내', '긴급 장애 즉시'],
    },
    ...
    <section id="contact" style={{ background: 'var(--bg)', position: 'relative', zIndex: 1 }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '100px 5%' }}>
        <div className="reveal">
          ...
          <p style={{ fontSize: '0.95rem', color: 'var(--text)', maxWidth: 520, lineHeight: 1.8 }}>전문 엔지니어가 직접 검토 후 24시간 내 회신드립니다.</p>
        </div>
              ...
              <button type="submit" disabled={status === 'sending'} style={{
                ...
                {status === 'idle' && (
                  <>
                    ...
                    <span style={{ display: 'block', fontSize: '0.72rem', fontWeight: 500, marginTop: 4, opacity: 0.92 }}>전문 엔지니어가 24시간 내 회신</span>
                  </>
                )}
              </button>
            </form>
