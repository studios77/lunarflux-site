...
type FcWindow = Window & { fcWidget?: { open?: () => void } }

/** 위젯이 도착할 때까지 기다렸다가 엽니다. 못 찾으면 false. */
async function openWhenReady(timeoutMs = 6000): Promise<boolean> {
  const started = Date.now()
  while (Date.now() - started < timeoutMs) {
    const fc = (window as FcWindow).fcWidget
    if (fc?.open) {
      fc.open()
      return true
    }
 ...
 *
 * FreshChat 위젯은 유휴 시간까지 로딩을 미루므로, 방문자가 이 버튼을 먼저
 * 누르면 `fcWidget` 이 아직 없을 수 있습니다. 그래서 바로 실패시키지 않고
 * 도착할 때까지 잠깐 기다립니다. 끝내 못 뜨면 — 스크립트 차단이나 네트워크
 * 문제 — 아래 이메일로 안내합니다. 눌러도 아무 일이 없는 버튼은 두지 않습니다.
 */
export default function FreshChatOpenButton() {
